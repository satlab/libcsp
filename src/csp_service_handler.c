...
static int do_cmp_clock(struct csp_cmp_message *cmp) {
	...
	csp_timestamp_t t;
	t.tv_sec = csp_ntoh32(cmp->clock.tv_sec);
	t.tv_nsec = csp_ntoh32(cmp->clock.tv_nsec);

	if ((t.tv_sec != 0) && (clock_set_time != NULL)) {
		clock_set_time(&t);
	}
	...
	cmp->clock.tv_sec = csp_hton32(t.tv_sec);
	cmp->clock.tv_nsec = csp_hton32(t.tv_nsec);
	return CSP_ERR_NONE;
